    ...
    render() {
        const listItems = this.props.linkList.map((item,index) => 
            <li key={item.name}> 
                { 
                    item.toLink ? 
                    <NavLink exact activeClassName="active" to={item.toLink}><i className="iconfont icon-star"></i>&nbsp;&nbsp;{item.name}</NavLink> : 
                    <div>
                        <p onClick={()=>{this.showItemNav(index)}}>
                            <i className="iconfont icon-dashujukeshihuaico-"></i>&nbsp;&nbsp;
                            <span>{item.name}</span>
                            <i className={`${item.childrenShow==='hide'?"":"up"} iconfont icon-xiangxiajiantou`}></i>
                        </p>
